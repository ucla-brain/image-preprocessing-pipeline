...
def get_voxel_sizes(is_mip):
    ...
    if is_mip:
        print()
    voxel_size_z = float(input(
        "what is the z-step size in µm?\n"
        f"{PrintColors.BLUE}hint: z-step is {600 if is_mip else 1} µm for the {'MIP' if is_mip else 'main'} "
        f"images generated by SmartSPIM{PrintColors.ENDC}\n").strip())
    tile_overlap_percent = float(input(
        f"what is the tile overlap in percent?\n"
        f"{PrintColors.BLUE}hint: for SmartSPIM 9, for others 16.2{PrintColors.ENDC}\n").strip())
    print(
    ...
    if not stitched_path.joinpath(f"{channel}_xml_import_step_5.xml").exists() or not continue_process_terastitcher:
        ...
        for step in [2, 3, 4, 5]:
            ...
            command += [
                ...
                f"--subvoldim={1 if objective == '40x' else 150}",  # Number of slices per subvolume partition
