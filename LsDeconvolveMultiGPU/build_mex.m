...
% Keivan Moradi, 2024-2025 (with ChatGPT-4o assistance)
    ...
    %% --------- 3) Build zlib-ng (consolidated flags) ---------
    cmake_common_flags_win = {policy_flag, ...
    ...
    cmake_common_flags_linux = cmake_common_flags_win;
    ...
    %% --------- 4) Build libtiff (consolidated flags) ---------
    cmake_common_flags_win = {policy_flag, '-DBUILD_SHARED_LIBS=OFF', ...
        '-DCMAKE_POSITION_INDEPENDENT_CODE=ON', ...
        '-DCMAKE_BUILD_TYPE=Release', ...
        '-Djbig=OFF', '-Djpeg=OFF', '-Dold-jpeg=OFF', '-Dlzma=OFF', '-Dwebp=OFF', ...
        '-Dlerc=OFF', '-Dpixarlog=OFF', '-Dlibdeflate=OFF', ...
    ...
    if isWin
        ...
        if debug
            mex_cpu_linux = [mex_cpu_linux, 'CFLAGS="$CFLAGS -O0 -g"', 'CXXFLAGS="$CXXFLAGS -O0 -g"', 'LDFLAGS="$LDFLAGS -g"'];
        else
