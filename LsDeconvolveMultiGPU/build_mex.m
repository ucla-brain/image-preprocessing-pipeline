% build_mex.m — static, LTO-enabled build of zlib-ng, zstd,
%               libtiff and all project MEX files for Linux & Windows
%
% Version : 2025-06-24  (no libdeflate — using zlib-ng for Deflate)
% Author  : Keivan Moradi  (with ChatGPT-o3 assistance)
% License : MIT
%
% -- Features --
%  • Builds zlib-ng and zstd in‐tree, statically, with full LTO
%  • Disables unwanted TIFF codecs (JPEG, JBIG, LZMA, WebP, LERC, PixarLog)
%  • Uses zlib-ng (-DZLIB_LIBRARY) for Deflate/Zip compression
...
%======================================================================
% 2) zlib-ng (zlib replacement for Deflate)
%======================================================================
...
if ~isfile(stamp)
    ...
    if ~exist(zstd_src,'dir')
        ...
        websave(tgz, sprintf('https://github.com/facebook/zstd/archive/refs/tags/v%s.tar.gz', zstd_v));
        untar(tgz, thirdparty); delete(tgz);
    end
    if ispc
        args = [ ...
...
%======================================================================
% 4) libtiff (disable JPEG, JBIG, LZMA, WebP, LERC, PixarLog)
%    — use zlib-ng for Deflate
%======================================================================
stamp = fullfile(libtiff_inst,'.built');
if ~isfile(stamp)
    fprintf('[libtiff] building… (no JPEG/JBIG/LZMA/WebP/LERC/PixarLog)\n');
    ...
    args = sprintf([ ...
      '-DBUILD_SHARED_LIBS=OFF ', ...
      '-DCMAKE_POSITION_INDEPENDENT_CODE=ON ', ...
      '-Djbig=OFF -Djpeg=OFF -Dold-jpeg=OFF -Dlzma=OFF -Dwebp=OFF -Dlerc=OFF -Dpixarlog=OFF ', ...
