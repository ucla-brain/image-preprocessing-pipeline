/*==============================================================================
  ...
  FEATURES:
    ...
    • Exception aggregation/reporting (main thread only).
...
static void writeSlice(
    ...
    // calculate image width/height based on isXYZ flag
    size_t width      = isXYZ ? dimX : dimY;
    size_t height     = isXYZ ? dimY : dimX;
    size_t sliceBytes = dimX * dimY * bytesPerSample;
    const uint8_t* srcPtr = volumeData + sliceIndex * sliceBytes;
...

    {
        ...
        // deflate predictor for better compression
        if (compression == COMPRESSION_DEFLATE) {
            TIFFSetField(tif, TIFFTAG_PREDICTOR,          PREDICTOR_HORIZONTAL);
            TIFFSetField(tif, TIFFTAG_ZIPQUALITY,         6);  // 1–9, 6 = default
        }
        ...
        for (uint32_t s = 0; s < numStrips; ++s) {
            ...
            for (uint32_t r = 0; r < rowsToWrite; ++r) {
                for (size_t c = 0; c < width; ++c) {
                    ...
                    size_t srcCol = isXYZ ? (rowStart + r) : c;
                    size_t srcOff = (srcRow + srcCol * dimX) * bytesPerSample;
                    size_t dstOff = (r * width + c) * bytesPerSample;
                    std::memcpy(&stripBuffer[dstOff], srcPtr + srcOff, bytesPerSample);
                }
            }
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    // 7) guard-clauses: output dirs exist & writable
    ...
    // 10) for collecting errors
    std::vector<std::string> errors;
    std::mutex              errorLock;

    // 11) atomic slice index dispatch
    ...
    // 13) join threads & report errors
    for (auto& th : workers) th.join();
    if (!errors.empty())
        mexErrMsgIdAndTxt("save_bl_tif:runtime", errors.front().c_str());

    // 14) optionally return input volume
    if (nlhs > 0)
