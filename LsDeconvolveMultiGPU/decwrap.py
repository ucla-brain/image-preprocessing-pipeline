...
def estimate_block_size_max(gpu_indices, num_workers, bytes_per_element=4, base_reserve_gb=2.5, per_worker_mib=160,
                            num_blocks_on_gpu=2):
    max_allowed = 2 ** 31
    try:
        result = subprocess.run(
...
def main():
    ...
    if user_specified_subset and not user_overrode_block_size:
        args.block_size_max = estimate_block_size_max(
            ...
            num_blocks_on_gpu=(100 if args.use_fft else 2) + (1 if args.lambda_damping else 0),
        )
