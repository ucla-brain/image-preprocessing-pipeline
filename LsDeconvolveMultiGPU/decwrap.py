...
def estimate_block_size_max(gpu_indices, num_workers, bytes_per_element=4, base_reserve_gb=0.75, per_worker_mib=160.0,
                            num_blocks_on_gpu=2):
    max_allowed = 2 ** 31 - 1
    try:
        result = subprocess.run(
...
def main():
    ...
    if user_specified_subset or not user_overrode_block_size:
        args.block_size_max = estimate_block_size_max(
            ...
            per_worker_mib=160 if args.use_fft else 422.5,
