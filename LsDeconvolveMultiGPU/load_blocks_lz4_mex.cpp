...
/*==============================================================================
 *                     1.  File-format constants & helpers
 *============================================================================*/
...
/*---- Header as written by save_lz4_mex ------------------------------------*/
struct FileHeader {
    uint32_t magic;
    uint8_t  dtype;
    uint8_t  ndims;
...
/* Parse & validate header; accept ndims 2 or 3 (Z-size may be 1) */
...
/* Column-major linear index (0-based) */
...
/*==============================================================================
 *                    2.  One brick-loading worker (functor)
 *============================================================================*/
struct BrickJob
{
    std::string file;        // path to *.lz4c
    /* brick bounds inside final volume (0-based inclusive) */
    uint64_t x0,y0,z0,  x1,y1,z1;
    uint64_t dimX, dimY, dimZ;   // full volume dims
    float*   volPtr;             // shared destination pointer
    ...
    void operator()() const
    {
        ...
        const uint64_t headerX = h.dims[0];
        const uint64_t headerY = h.dims[1];
        const uint64_t headerZ = (h.ndims == 3) ? h.dims[2] : 1ULL;

        if (brickX!=headerX || brickY!=headerY || brickZ!=headerZ)
            throw std::runtime_error(file + ": dims in header ≠ expected brick dims");

        /* temp buffer = full uncompressed brick */
        std::vector<float> uBuffer(brickX * brickY * brickZ);
        ...
        for (uint32_t c=0; c<h.numChunks; ++c)
        {
            ...
            if (compB  > 0x7FFFFFFF || uncomp > 0x7FFFFFFF)
                throw std::runtime_error(file + ": chunk > 2 GB (corrupt?)");

            std::vector<char> cBuf(compB);
            freadExact(fp.get(), cBuf.data(), compB, ("reading chunk of "+file).c_str());

            const int decoded = LZ4_decompress_safe(cBuf.data(),
                                                    dst + offset,
        ...
        if (offset != h.totalUncompressed)
            throw std::runtime_error(file + ": total size mismatch after decompress");

        /*--- Copy into final volume (row-wise) -----------------------------*/
        ...
        /* fp closed automatically by unique_ptr */
    }
};

/*==============================================================================
 *               3.  MEX gateway – parse args, launch jobs
 *============================================================================*/
                 ...
                 int nrhs, const mxArray* prhs[])
{
    ...
    if (N == 0) mexErrMsgTxt("Filename list is empty");
    ...
    if (mxGetM(p1mx)!=N || mxGetN(p1mx)!=3 ||
        ...
        mexErrMsgTxt("p1 and p2 must be N×3 matrices");

    /* Helper to fetch 1-based index as uint64 ----------------------------*/
    ...
    /* Allocate output volume (single) ------------------------------------*/
    mwSize mdims[3] = { static_cast<mwSize>(dimX),
                        static_cast<mwSize>(dimY),
                        static_cast<mwSize>(dimZ) };
    mxArray* volMx = mxCreateNumericArray(3, mdims, mxSINGLE_CLASS, mxREAL);
    if (!volMx) mexErrMsgTxt("Out of memory allocating output volume");
    ...
    /* Launch async jobs ---------------------------------------------------*/
    std::vector<std::future<void>> futs; size_t next=0;
    auto launch = [&](size_t k){ futs.emplace_back(std::async(std::launch::async, jobs[k])); };

    const size_t initial = std::min<size_t>(jobs.size(), static_cast<size_t>(maxThreads));
    for (; next<initial; ++next) launch(next);

    /* Poll / recycle threads until all jobs done -------------------------*/
    while (!futs.empty())
    {
        for (auto it = futs.begin(); it != futs.end(); )
        {
            if (it->wait_for(std::chrono::milliseconds(20)) == std::future_status::ready)
            {
                it->get();                           // re-throw any exception
                it = futs.erase(it);
                if (next < jobs.size()) launch(next++);
            }
            else ++it;
        }
    }
