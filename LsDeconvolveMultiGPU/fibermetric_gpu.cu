// fibermetric_gpu_minvram.cu
// VRAM-optimized, high-accuracy Frangi/Ferengi vesselness (fibermetric) CUDA MEX
// - After eigenvalue computation, frees Hessian/cross-derivative buffers
...
{
    size_t kernelBytes = kernelLen * sizeof(float);
    ...
    int nRows, int nCols, int nSlices)
{
    ...
    // 3D indices for column-major layout
    int row   = idx % nRows;
    int col   = (idx / nRows) % nCols;
    int slice = (idx / (nRows * nCols));
...
//-------------------- 5-point Second Derivative (fmaf, precompute sqs) ----------------------
template<int AXIS>
    ...
    int nRows, int nCols, int nSlices)
{
    ...
    float v = fmaf( -1.0f, input[linearIndex3D(rr[0], cc[0], ss[0], nRows, nCols)], 0.f);
    v = fmaf( 16.0f, input[linearIndex3D(rr[1], cc[1], ss[1], nRows, nCols)], v);
    v = fmaf(-30.0f, input[linearIndex3D(rr[2], cc[2], ss[2], nRows, nCols)], v);
    v = fmaf( 16.0f, input[linearIndex3D(rr[3], cc[3], ss[3], nRows, nCols)], v);
    v = fmaf( -1.0f, input[linearIndex3D(rr[4], cc[4], ss[4], nRows, nCols)], v);
...
)
{
    ...
    // Sato's vesselness: Use only if "vessel" eigenvalues match polarity
    ...
    if (isVessel) {
        ...
        // Exponentials, all with precomputed reciprocal (float, from double on host)
