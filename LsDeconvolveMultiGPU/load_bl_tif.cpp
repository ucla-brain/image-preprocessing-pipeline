...
struct LoadTask {
    ...
    LoadTask() = default;
    LoadTask(
        uint32_t inY, uint32_t inX, size_t outY, size_t outX,
...
// --- Helper Prototypes ---
InputArgs parse_inputs(int nrhs, const mxArray* prhs[]);
void check_tiff_metadata(const std::vector<std::string>& fileList, uint32_t roiY0, uint32_t roiX0, uint32_t roiH, uint32_t roiW, uint16_t& outBitsPerSample);
void* create_output_array(mxArray*& plhs0, mxClassID outType, size_t outH, size_t outW, size_t numSlices);
std::vector<LoadTask> create_tasks(const std::vector<std::string>& fileList, uint32_t roiY0, uint32_t roiX0, uint32_t roiH, uint32_t roiW, size_t pixelsPerSlice, bool transpose);
inline size_t computeDstIndex(const LoadTask& task, uint32_t row, uint32_t col) noexcept;
void readSubRegionToBuffer(const LoadTask& task, TIFF* tif, uint8_t bytesPerPixel, std::vector<uint8_t>& blockBuf, std::vector<uint8_t>& tempBuf);
...
// --- Main Producer/Consumer Parallel Decode ---
void parallel_decode_and_copy(const std::vector<LoadTask>& tasks, void* outData, size_t bytesPerPixel) {
    ...
    for (size_t t = 0; t < threadPairCount; ++t) {
        ...
        consumerThreads.emplace_back([&, t] {
            ...
            while (true) {
                ...
                // Fast path for no-transpose: bulk copy
                if (!task.transpose) {
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    try {
        ...
        InputArgs args = parse_inputs(nrhs, prhs);

        // 2. Metadata checks: size, depth, ROI, grayscale
