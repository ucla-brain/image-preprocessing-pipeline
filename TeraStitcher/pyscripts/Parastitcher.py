...
def worker(input_file):
    ...
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    if debug_level > 0:
        execution_string = prefix + list(input_file.values())[0] + " > " + "output_" + str(
            list(input_file.keys())[0]) + ".out"
    else:
...
def do_additional_partition(nprocs, nrows, ncols, n_ss):
    ...
    if n_ss >= 2 * nprocs:  # dataset has not to be further partitioned
        ...
        print("Dataset partitioned in", int(floor(nrows / 2)), "x", int(floor(ncols / 2)), "x", n_ss, "=",
              int(floor(nrows / 2)) * int(floor(ncols / 2)) * n_ss, "partitions")
...
def read_params():
    ...
    file_in = read_item(params, '--origin=',
                        os.getcwd() + '/__dims__.txt')  # changed by Giulio: the --origin option is not needed any longer
    ...
    os.remove(
        os.getcwd() + '/__dims__.txt')  # added by Giulio to remove the temporary file containing image size information
    ...
    if isotropic:
        ...
        h = 0
    max_res_D = max_res - h
    print("vxl_V, vxl_H, vxl_D, isotropic, h, max_res, max_res_D :", vxl_V, vxl_H, vxl_D, isotropic, h, max_res,
          max_res_D)
    ...
    return (input_name, output_name, wb1, wb2, wb3, sfmt, dfmt, iresolutions, max_res, isotropic, max_res_D, params,
            last_string, height, width, depth, bytes_x_chan, n_chans)
...
def eliminate_double_quote(inpstring):
    ...
    if (inpstring[0] == '"') and (inpstring[len_str - 1] == '"') or (inpstring[0] == "'") and (
            inpstring[len_str - 1] == "'"):
        newstring = inpstring[1:len_str - 1]
    return newstring


def generate_first_command(input_name, output_name, wb1, wb2, wb3, sfmt, dfmt, iresolutions, max_res, params,
                           last_string):
    """
...
def generate_final_command(input_name, output_name, wb1, wb2, wb3, sfmt, dfmt, iresolutions, max_res, params,
                           last_string):
    """
...
def generate_parallel_command(start_list, end_list, input_name, output_name, wb1, wb2, wb3, sfmt, dfmt, iresolutions,
                              max_res, params, last_string):
    """
...
def create_commands(gi_np, info=False):  # 2016-12-10. Giulio. @ADDED parameter gi_np: desired parallelism
    ...
    (input_name, output_name, wb1, wb2, wb3, sfmt, dfmt, iresolutions, max_res, isotropic, max_res_D, params,
     last_string, height, width, depth, bytes_x_chan, n_chans) = read_params()
    print("#" * 80)
    ...
    size_1 = create_sizes(depth, wb1,
                          max_res_D)  # max_res_D = max_res if isotropic is not set of if voxels are isotropic
    ...
    (partition_depth, partition_width, partition_height) = ctrl_parallelism(eliminate_double_quote(sfmt),
                                                                            eliminate_double_quote(dfmt))
    print('--------> ', eliminate_double_quote(sfmt), eliminate_double_quote(dfmt), partition_depth, partition_width,
          partition_height)
    ...
    if info:
        ...
        (order, start_list, end_list, len_arr) = sort_start_end(start_1, start_2, start_3, end_1, end_2, end_3, size_1,
                                                                size_2, size_3)
        # Generate the string to initialize parallel computation
        first_string = generate_first_command(input_name, output_name, wb1, wb2, wb3, sfmt, dfmt, iresolutions, max_res,
                                              params, last_string)
        # Generate the list of parallel command lines
        list_string = generate_parallel_command(start_list, end_list, input_name, output_name, wb1, wb2, wb3, sfmt,
                                                dfmt, iresolutions, max_res, params, last_string)
        # Generate the string to merge all metadadata
        final_string = generate_final_command(input_name, output_name, wb1, wb2, wb3, sfmt, dfmt, iresolutions, max_res,
                                              params, last_string)
...
if __name__ == '__main__':
    ...
    if myrank == 0:
        if step2:
            ...
            #         nrows = 11
            #         ncols = 12
            (p_r, p_c) = do_additional_partition(float(nprocs - 1), float(nrows), float(ncols),
                                                 float(n_ss))  # master should not be counted
            ...
            if len(r_start) > 1 or len(c_start) > 1:
                ...
                for i in range(len(r_start)):
                    for j in range(len(c_start)):
                        gr_dir_names.append(
                            'gr_R[' + str(r_start[i]) + ',' + str(r_end[i]) + ']_C[' + str(c_start[j]) + ',' + str(
                                c_end[j]) + ']/')
            ...
            if len(r_start) == 1 and len(c_start) == 1:  # no groups
                ...
                for i in range(n_ss):
                    ...
                    tmp_string = execution_flag + ' ' + params_str + ' --projout=' + new_output_name + '-' + str(
                        start_dict[i]).zfill(6) + '-' + str(end_dict[i]).zfill(6) + '.xml' + ' --subvoldim=' + str(
                        new_params[i]) + ' --D0=' + str(start_dict[i]) + ' --D1=' + str(
                        end_dict[i]) + ' --noprogressbar'
                ...
                for r in range(len(r_start)):
                    for c in range(len(c_start)):
                        ...
                        for i in range(n_ss):
                            ...
                            end_dict.update({i: end_tmp - 1})
                            tmp_string = execution_flag + ' ' + params_str + ' --projout=' + new_output_name + '-' + str(
                                start_dict[i]).zfill(6) + '-' + str(end_dict[i]).zfill(
                                6) + '.xml' + ' --subvoldim=' + str(new_params[i]).zfill(6) + ' --D0=' + str(
                                start_dict[i]).zfill(6) + ' --D1=' + str(end_dict[i]).zfill(6)
                            tmp_string = tmp_string + ' --R0=' + str(r_start[r]) + ' --R1=' + str(
                                r_end[r]) + ' --C0=' + str(c_start[c]) + ' --C1=' + str(
                                c_end[c]) + ' --noprogressbar --parallel'
                            if c < (
                                    len(c_start) - 1):  # it not the last group of columns: disable displacement computation of last column
                                tmp_string = tmp_string + ' --disable_last_col'
                            if r < (
                                    len(r_start) - 1):  # it not the last group of columns: disable displacement computation of last column
            ...
            if len(r_start) == 1 and len(c_start) == 1:  # no groups
                ...
                if debug_level > 0:
                    execution_string = prefix + 'mergedisplacements -d=' + tmp_xml_dir[0:slash_pos] + ' -o=' + \
                                       tmp_out_name[0] + tmp_out_name[1] + '.xml' + ' > ' + 'ouput_mdispls.out'
                else:
                    execution_string = prefix + 'mergedisplacements -d=' + tmp_xml_dir[0:slash_pos] + ' -o=' + \
                                       tmp_out_name[0] + tmp_out_name[1] + '.xml'
                print(execution_string)
                os.system(execution_string)
                ...
                for dname in gr_dir_names:
                    ...
                    if debug_level > 0:
                        execution_string = prefix + 'mergedisplacements -d=' + tmp_out_name[0] + dname[
                                                                                                 0:slash_pos] + ' -o=' + \
                                           tmp_out_name[0] + 'tmp/' + tmp_out_name[
                                               1] + suffix + '.xml' + ' > ' + 'ouput_' + suffix + '.out'
                    else:
                        execution_string = prefix + 'mergedisplacements -d=' + tmp_out_name[0] + dname[
                                                                                                 0:slash_pos] + ' -o=' + \
                                           tmp_out_name[0] + 'tmp/' + tmp_out_name[1] + suffix + '.xml'
                    print(execution_string)
                    os.system(execution_string)
                ...
                slash_pos = len(tmp_xml_dir) - 1
                if debug_level > 0:
                    execution_string = prefix + 'mergedisplacements --mgroups -d=' + tmp_xml_dir[0:slash_pos] + ' -o=' + \
                                       tmp_out_name[0] + tmp_out_name[1] + '.xml' + ' > ' + 'ouput_mgroups.out'
                else:
                    execution_string = prefix + 'mergedisplacements --mgroups -d=' + tmp_xml_dir[0:slash_pos] + ' -o=' + \
                                       tmp_out_name[0] + tmp_out_name[1] + '.xml'
                print(execution_string)
