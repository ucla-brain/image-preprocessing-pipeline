clc; clear all;
global cSpt RunOnWindows BlankTifFile RootPathOfData PathTarget FramesizeRow FramesizeCol XYOffset ZOffset;
RunOnWindows = true; 
RootPathOfData = 'G:\Test';
PathTarget = 'G:\Test_Target';
...
ZOffset = 20;
    ...
    for kY = 1:size(YFolders, 2)                    % #Cols
        ...
        sX = num2str(kY*XYOffset, '%06d');                % Tartet folder name
        for kX = 1:size(XFolders, 2)                % #Rows
            XFolder = XFolders{kX};                 % Source folder name
            sY = num2str(kX*XYOffset, '%06d');            % Target folder name
            ...
            for kZ = 1:size(ZFolders, 2)            % #Z
                ZFolder = ZFolders{kZ};
                sZ = num2str((kZ - 1)*ZOffset, '%06d');  % Target #Z, Start from 000000
